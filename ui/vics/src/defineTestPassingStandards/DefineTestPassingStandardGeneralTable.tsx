...
function DefineTestPassingStandardGeneralTable() {

  const [pageSize] = useState(15);
  const [page, setPage] = useState(1);
  const [isDisablePassingStandard, setIsDisablePassingStandard] = useState(false);
  const [isReactivatePassingStandard, setIsReactivatePassingStandard] = useState(false);
  const [isActive, setIsActive] = useState(true);

  const handleSwitchChange = () => {
    if (isActive) {
      setIsDisablePassingStandard(true);
    } else {
      setIsReactivatePassingStandard(true);
    }
  ...
  return (
    <div>
      <div className="w-full overflow-hidden  ">
        <Table radius="none" shadow="sm" classNames={{ wrapper: "p-0   w-full overflow-hidden" }} bottomContent={
          <div className="flex  justify-end pt-0 table-auto ">
            {uservalidityPeriodData.length > 15 ? (
              <Pagination
                isCompact
                showControls
                showShadow
                ...
                classNames={{
                  wrapper:
                    "gap-0  h-8  rounded-none border border-divider",
      ...
      {isDisablePassingStandard && (
        <DisablePassingStandardPopUp setIsDisablePassingStandard={setIsDisablePassingStandard} />
      )}
      {isReactivatePassingStandard && (
        <ReactivePassingStandardPopUp setIsReactivatePassingStandard={setIsReactivatePassingStandard} />
      )}
    </div>
  );
}

export default DefineTestPassingStandardGeneralTable;
