...
export default function ReactiveDefectPopup({ setIsReactivateDefect,systemName }) {
  const setIsReactivate=()=>{setIsReactivateDefect(false)}
  return (
    <Modal
      isOpen={true}
      onClose={setIsReactivate}
      isDismissable={false}
      size="full"
      className="w-[35%]  h-[40%] "
    >
        ...
        {(onClose) => (
          <>
            <div className="bg-lightGreen w-[100%]  text-[12px] p-1 font-bold text-black text-center h-8">
              Reactive Defect - System Code
            </div>

            <ModalBody>
              <div className=" text-center text-black  p-1 text-inputTxt mt-2 font-bold">
                Do you confirm to reactivate this Defect - System Code?
              </div>
